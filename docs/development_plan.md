...
이 문서는 `commtool` 앱을 실제 코드로 구현할 때 사용할 파일명, 함수명, 변수명, 사용처, 용도를 함께 기록하는 개발 기준 문서입니다. `docs/app_dev_def.md`의 MVP 기능 정의서를 기준으로 하며, 핵심 도메인은 임시로 `Item/items`를 사용합니다. 실제 앱 도메인명이 확정되면 도메인명과 기능명을 이 문서에 맞춰 갱신합니다.
...
| 문서 | 확인 목적 | 현재 상태 |
| --- | --- | --- |
| `docs/app_dev_def.md` | 앱 목적, 사용자, 핵심 기능 확정 | 작성됨 |
| `docs/requirements.md` | MVP 범위와 우선순위 확정 | 작성됨 |
| `docs/functional_spec.md` | 기능별 입력, 처리, 결과 확정 | 작성됨 |
| `docs/data_model.md` | 저장할 데이터와 관계 확정 | 작성됨 |
| `docs/api_spec.md` | API 요청/응답 계약 확정 | 작성됨 |
| `docs/ui_ux_flow.md` | 화면 흐름과 상태 정의 | 작성됨 |
...
| 리스크 | 영향 | 대응 |
| --- | --- | --- |
| 실제 도메인명 미확정 | `Item/items` 명칭이 임시로 유지됨 | 앱 목적 확정 후 실제 도메인명으로 일괄 교체 |
...
다만 실제 앱 도메인명, 인증 방식, 데이터 저장 방식은 아직 최종 확정 전이므로 실제 개발 착수 전에는 다음 항목을 다시 확인해야 합니다.
